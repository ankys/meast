...
<カラテオドリの拡張定理>
#block[
集合$X$の部分集合$A$に対して$m^(‾) (A)$が次を満たすとき、$m^(‾)$を$X$の_外測度_という。

+ 任意の部分集合$A subset X$に対して$m^(‾) (A) in RR^(‾)_(+)$である。

+ 空集合について$m^(‾) (nothing) = 0$が成り立つ。

+ 任意の二つの部分集合$A, B subset X$に対して$A subset B$ならば$m^(‾) (A) <= m^(‾) (B)$が成り立つ。

+ 任意の可算個の部分集合$A_i subset X$に対して$m^(‾) (union.big_i A_i) <= sum_i m^(‾) (A_i)$が成り立つ。

]
#block[
集合$X$のいくつかの部分集合$D$（その集合を$cal(D)$とおく）に拡張非負値$tilde(m) (D) in RR^(‾)_(+)$を対応させる。
ここで$tilde(m) (nothing) = 0$と$X in cal(D)$を仮定して、
$X$の部分集合$A$に対して可算個の$D_i in cal(D)$で被覆することで
\$\$\\bar{m}(A) = \\inf\\qty{ \\sum\_i \\tilde{m}(D\_i) \\mid \\bigcup\_i D\_i \\supset A }\$\$
と定義すると、$m^(‾)$は$X$の外測度になる。

]
#block[


+ 部分集合$A subset X$に対して被覆は存在するので$m^(‾) (A) in RR^(‾)_(+)$である。

+ 空集合$nothing$について$nothing in cal(D)$で被覆されるので$m^(‾) (nothing) <= tilde(m) (nothing) = 0$である。

+ $A subset B$のとき$B$の被覆は$A$の被覆でもあるので$m^(‾) (A) <= m^(‾) (B)$が成り立つ。

+ 可算個の部分集合$A_i subset X$に対して、それぞれに可算被覆をとってそれら全ては可算被覆なので$m^(‾) (union.big_i A_i) <= sum_i m^(‾) (A_i)$が成り立つ。
...
$ m^(‾) (B sect A)+m^(‾) (B sect A^c) = m^(‾) (B) $
を満たす時、そのような$A$全体を$cal(F)$として$m$を$m^(‾)$の$cal(F)$への制限とする時、
...
#block[
 空集合$A = nothing$について、
$ m^(‾) (B sect A)+m^(‾) (B sect A^c) = m^(‾) (nothing)+m^(‾) (B) = m^(‾) (B) $
なので$nothing in cal(F)$であり、$m (nothing) = m^(‾) (nothing) = 0$である。

集合$A in cal(F)$について、
$ m^(‾) (B sect A^c)+m^(‾) (B sect A) = m^(‾) (B sect A)+m^(‾) (B sect A^c) $
より$A^c in cal(F)$である。

可算個の前に二つの集合$A_1, A_2 in cal(F)$に対して
$ m^(‾) (B sect (A_1 union A_2))+m^(‾) (B sect (A_1 union A_2)^c) & = m^(‾) ((B sect A_1) union (B sect A_1^c sect A_2))+m^(‾) (B sect A_1^c sect A_2^c)\
 & <= m^(‾) (B sect A_1)+m^(‾) (B sect A_1^c sect A_2)+m^(‾) (B sect A_1^c sect A_2^c) = m^(‾) (B) . $
よって$A_1 union A_2 in cal(F)$である。
可算個の集合$A_1, A_2, A_3, dots in cal(F)$に対して、互いに交わりがない場合は$A = union.big_i A_i$について
$ m^(‾) (B) & = m^(‾) (B sect A_1)+m^(‾) (B sect A_1^c) = m^(‾) (B sect A_1)+m^(‾) (B sect A_2)+m^(‾) (B sect A_1^c sect A_2^c)\
 & = dots = m^(‾) (B sect A_1)+dots+m^(‾) (B sect A_n)+m^(‾) (B sect A_1^c sect dots sect A_n^c)\
 & >= m^(‾) (B sect A_1)+dots+m^(‾) (B sect A_n)+m^(‾) (B sect A^c) $
で$n arrow.r oo$として
$ m^(‾) (B) >= sum_i m^(‾) (B sect A_i)+m^(‾) (B sect A^c) >= m^(‾) (union.big_i B sect A_i)+m^(‾) (B sect A^c) >= m^(‾) (B sect A)+m^(‾) (B sect A^c) $
...
$ m (A) = m^(‾) (A) >= sum_i m^(‾) (A_i)+m^(‾) (nothing) = sum_i m (A_i) . $
一方で外測度の要件より不等号は等号として成立するので、$m$は測度である。

完備性について$m (A) = 0$, $C subset A$とすると、$B subset X$に対して
$ m^(‾) (B sect C)+m^(‾) (B sect C^c) <= m^(‾) (B sect A)+m^(‾) (B sect C^c) <= m^(‾) (A)+m^(‾) (B) = m^(‾) (B) . $
...
まず、外測度$m^(‾)$を、部分集合$A subset X$に対して可算個の可測集合$A_i in cal(A)$で被覆することで
...
あとは$cal(F) supset cal(A)$と任意の$A in cal(A)$に対して$m^(‾) (A) = m_0 (A)$を示せば定理が証明される。

このうち$cal(F) supset cal(A)$は$A in cal(A)$が$A in cal(F)$つまり$B subset X$に対して
$ m^(‾) (B sect A)+m^(‾) (B sect A^c) <= m^(‾) (B) $ を示せばよい。
$m^(‾) (B) = oo$ならば直ちに従うので、$m^(‾) (B) < oo$の場合を考える。
正の数$epsilon > 0$に対して外測度の定義より$B_i in cal(A)$が存在して$union.big_i B_i supset B$かつ
$ sum_i m_0 (B_i) <= m^(‾) (B)+epsilon $ となる。
ここで$B sect A subset union.big_i (B_i sect A)$と$B sect A^c subset union.big_i (B_i sect A^c)$より
$ m^(‾) (B sect A)+m^(‾) (B sect A^c) <= sum_i m_0 (B_i sect A)+sum_i m_0 (B_i sect A^c) = sum_i (m_0 (B_i sect A)+m_0 (B_i sect A^c)) $
ここで$m_0$は有限加法性を持っているので、
$ m^(‾) (B sect A)+m^(‾) (B sect A^c) <= sum_i m_0 (B_i) <= m^(‾) (B)+epsilon . $
したがって$cal(F) supset cal(A)$がわかる。

続いて$A in cal(A)$に対して$m^(‾) (A) = m_0 (A)$を示すが、外測度の定義から$m^(‾) (A) <= m_0 (A)$は自明なので$m_0 (A) <= m^(‾) (A)$を示す。
...
よって$cal(C)$が単調族であることを示せば、単調族定理より$cal(F) = sigma (cal(A)) subset cal(C)$なので、すべての$A in cal(F)$に対して$m_1 (A sect X_n) = m_2 (A sect X_n)$で$n arrow.r oo$として$m_1 (A) = m_2 (A)$を得る。
